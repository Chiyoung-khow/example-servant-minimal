{ pkgs }: with pkgs; let
  ...
  in runCommand "ghcid-test" { buildInputs = [ makeWrapper ]; } ''
    makeWrapper ${ghcid} ${out} --add-flags "--command='cabal repl test:test-servant' --test 'Main.main'"
  '';

in mkShell {
  buildInputs =  haskellPackages.example-servant-minimal.env.nativeBuildInputs ++
                 [ ghcCharged
                   ghcid-bin
                   ghcid-test
                   cabal-install
