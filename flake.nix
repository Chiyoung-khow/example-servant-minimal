{
  description = "example-servant-minimal";
  ...
  outputs =
    inputs@{ self, nixpkgs, flake-compat, flake-utils, ... }:
    {
      ...
      in rec {
        defaultPackage = pkgs.haskellPackages.example-servant-minimal;
        devShell = import ./develop.nix { inherit pkgs; };

        apps = let
          ghcEnv = pkgs.haskellPackages.ghcWithPackages (p: [ p.example-servant-minimal ]);
        in {
          jupyter = {
            type = "app";
